...
**ملحوظة على الأسماء قبل ما نبدأ.** لما أقول tab0 / tab4 / tab6 / tab8 / tab9 / tab10 أنا أقصد **ملفات CSV** طالعة من مرحلة التحليل (Stage 4)، مش جداول الورقة: `tab0_baselines.csv` (خطوط الأساس)، `tab4_mcnemar.csv` (الـ 260 اختبار)، `tab6_perclass.csv` (precision/recall لكل فئة)، `tab8_prompt_sensitivity.csv`، `tab9_evaluability.csv` (الـ 87 خلية والبوابة)، `tab10_like_for_like.csv`. جداول الورقة نفسها بأرقام رومانية (Table I–V) وبنقول عليها "الجدول 4" مثلًا.

وتذكير سريع بكلمة **regime** (نظام التقييم): هي بتقول الإنكودر اتقيّم تحت أي نوع انزياح — `in_domain` (تدريب واختبار من نفس الكوربس بـ 5 طيّات)، `cross_project` (اختبار على مشاريع ما شافهاش)، `cross_dataset` (تدريب على كوربس واختبار على التاني). الـ LLM ما بيتدربش أصلًا، فخليته في tab9 نظامها مكتوب `prompted` — ودي هي نفس الخلية اللي بتتقارن مع الإنكودر في التلات أنظمة.
...
حاجتين في الرسمة نفسها ممكن يلخبطوكي: كلمة *exemplar* وعبارة *the requirement under test* مكتوبين بخط **مائل عادي** مش monospace، لأنهم **placeholders** — يعني مكانهم بيتحط نص متطلّب حقيقي، مش دول كلام بيتبعت للموديل. وكمان `Answer: FR` مرسوم على يمين نفس السطر في الشكل عشان يوفّر مساحة في العمود، لكن في البرومبت الفعلي هو **سطر لوحده** تحت المتطلّب (شوفي الكود: `Requirement:\n"""{t}"""\nAnswer: {y}`).
...
- عشان تقارني: تعليمة **base** بتاعة الأمن بالحرف هي `Decide whether it is SECURITY-related (authentication, access control, confidentiality, integrity, encryption, auditing) or NON-SECURITY.` — يعني هي أصلًا بتعدّد ستة أمثلة.
...
خدي بالك من نقطة مهمة جدًا هتظهر في نتائج RQ2، بس لازم تتقال بدقة: **في تاسك الأمن بس** الـ terse سؤال نعم/لا ("Is this requirement security-related?")؛ في FR/NFR الـ terse سؤال اختيار ("Is this requirement functional or non-functional?")، وفي الأنواع الفرعية سؤال "Which quality attribute...". وده بالظبط اللي بيبان في tab8: فرق الصياغات عند Gemma على FR/NFR **0.022** بس، بينما على الأمن **0.345** (PROMISE) و**0.485** (SecReq) — يعني الأثر الكارثي مربوط بصيغة النعم/لا في تاسك الأمن، مش بالـ terse في المطلق. الـ base والـ verbose بيقولوا "قرّري بين X و Y"، لكن terse بتقول "هل هو X؟". الورقة بتقول إن ده اللي خلّى Gemma-2-2B تجاوب security على 96.7% من عناصر SecReq و51.3% من PROMISE_exp (بينما النسبة الحقيقية 39.7% و12.7%) — الأرقام دي من قسم الـ Discussion في الورقة ومحسوبة على عينة الـ 300، و"النسبة الحقيقية" يعني نسبة العناصر اللي الـ label بتاعها فعلًا security جوّه العينة دي.

**وقفة تلمّ تلات أرقام هيلخبطوكي:** هتشوفي نسبة الأمن في PROMISE مكتوبة **12.9%** و**12.6%** و**12.7%** في أماكن مختلفة، وكلهم نفس الحاجة بمقامات مختلفة: 12.9% = 125 ÷ 968 (الكوربس كله)، 12.6% = 121 ÷ 960 (بعد ما اتشالت الـ 8 أمثلة few-shot)، و12.7% = 38 ÷ 300 (جوّه عينة الـ 300 بتاعة دراسة الصياغة). فلو لقيتي رقم منهم في مكان، بصّي الأول على المقام هو إيه، فنزل الـ macro-F1 بتاعها على SecReq من 0.827 (base) لـ 0.343 (terse) ورجع 0.828 (verbose): فرق 0.485. مش هنفصّل ده هنا (ده قسم النتائج)، بس لازم تعرفي إن الفرق بين الصياغات هو "الكتلة الأولى بس"، وعقد الإجابة ثابت في التلاتة — وده اللي كابشن الشكل 2 بيأكّده.

الدراسة الفرعية دي اتعملت على موديلين بس (Qwen2.5-7B وGemma-2-2B) على التاسكات الثنائية، وعلى **الـ core subset**: 300 عنصر لكل خلية ثنائية (عمود `n_paired` في tab8)، و218 / 200 / 200 على all-11 / top-6 / top-4. (ليه 300؟ نفس السبب اللي في جزء العينات: 3 × 300 = 900 = الحصة اليومية المجانية لمزوّد واحد.) ليه اتنين مش الستة؟ الكود بيقول: عشان النتيجة ما تبقاش "artefact" لموديل واحد، لكن مش كل الستة لأن ده سؤال ثانوي والميزانية أولى بيها تغطية الـ few-shot. (على الـ sub-types اتعملت بموديل واحد بس، Qwen2.5-7B.) وصياغة الـ terse هناك مش نعم/لا خالص، هي سؤال اختيار من قايمة: `Which quality attribute does this non-functional requirement describe? Reply with exactly one of: ...`. والنتيجة إن فرق الصياغات على الأنواع الفرعية تافه: 0.0359 على all-11 (0.606 / 0.5905 / 0.6264 على 218 عنصر)، 0.0268 على top-4، و0.0439 على top-6 — وده اللي بيخلّي **وسيط** الفروق كله 0.044 بالرغم من الطرف الأقصى 0.485.
...
- الـ few-shot كله اتعمل على **الـ core subset بس** (300 عنصر ثنائي؛ 218/200/200 للأنواع الفرعية)، عشان k = 0 و2 و4 و8 يبقوا على نفس العناصر بالظبط فينفع نقارنهم بـ McNemar. الـ zero-shot اتعمل كمان على الإطار الكامل (960/960/436) للموديلات المحلية، وده اللي في جداول RQ1.
...
- الاختيار **deterministic** (ثابت) على خطوتين: (1) حوض الأمثلة (4 لكل فئة) بيتسحب مرة واحدة بـ `random_state = 42`؛ (2) عند كل k الكود بيلف على الفئات بالترتيب الأبجدي (FR قبل NFR، non-security قبل security) وياخد واحد واحد بالدور. الخطوة الأولى هي اللي بتضمن التكرار، والتانية هي اللي بتضمن التوازن بين الفئات.
...
- الموديلات المستضافة (Llama-3.3-70B) والتجارية (Gemini) اتشغّلت **zero-shot بس**. السبب اللي الورقة بتقوله: حدود الطلبات (rate limits) في الطبقات المجانية للمزوّدين — (والكود بيقولها صراحة في مقدمة المرحلة 3: "Hosted/API models run zero-shot only, **deliberately**: their capped free daily quota is spent entirely on the comparable zero-shot core condition" — يعني الحصة المجانية اتصرفت كلها على الشرط الأساسي القابل للمقارنة.)
...
**خطوة 0 (تحضير).** نظّف الرد: لو فيه `<think>...</think>` (بعض الموديلات بتكتب تفكيرها جواها) شيله؛ لو فيه `<think>` مفتوحة من غير قفلة، فالرد اتقطع في نص التفكير وبيتحسب فاضي.
**خطوة 0 (تكملة).** حوّل لحروف صغيرة ودوّر على أشكال كل label **على حدود الكلمات**. للثنائي: جانب `NFR` بيتعرّف بـ `nfr` / `non-functional` / `nonfunctional` / `non functional`، وجانب `FR` بـ `fr` / `functional` (بس مش لو قبلها `non`). لأمن المتطلّبات: `non-security` / `non security` / `nonsecurity` مقابل `security` (بس مش لو قبلها `non`).
**قاعدة 1 — الـ label اللي بيفتح الرد بيكسب**:: لو رد الموديل بيبدأ (الموضع 0) بأحد الجانبين، ده القرار، حتى لو الشرح اللي بعده ذكر الجانب التاني.
**قاعدة 2 — لو مفيش label في الأول، لازم يكون فيه جانب واحد بس مذكور في أي مكان**؛ ساعتها ده القرار.
**وإلا** — الرد ذكر **الاتنين** أو **ولا واحد** — بيتحسب **unparseable** (غير قابل للتحليل) **وبيتسجّل غلط**، مش بيتحذف.
...
| 6 | `<think>The requirement mentions login so it is prob` (اتقطع عند حد الـ 12 توكن، فمفيش `</think>`) | خطوة التنظيف بترجّع نص فاضي (تفكير مقطوع = مفيش إجابة) → ولا label | unparseable → **غلط**. |

الرد رقم 6 ده بالذات هو اللي بيربط حد الـ `max_new_tokens = 12` بالـ parser: الحد بيمنع الموديل يسرح، بس لو الموديل بدأ يفكّر بصوت عالي، التقطيع بيخلّي رده بلا إجابة — وبيتحسب غلط، مش بيتشال.
...
الـ 18 صف دول مين بالظبط؟ من المخزن الثنائي: **17 منهم من Nemotron وواحد من Qwen2.5-7B**، وكلهم نصهم من نوع "We need to decide if the requirement is functional or non-functional" واتقطع عند حد الـ 12 توكن. الـ parser القديم كان بيطلّعهم NFR (لأنها أطول)، ومسودّة تانية كانت بتطلّعهم FR (لأنها الأول في الجملة) — دلوقتي الاتنين مرفوضين. وده جزء من السبب إن Nemotron في tab9 على FR/NFR نسبة الـ parse بتاعته **0.0** (صفر من 21).

**ليه صارم كده؟** الورقة بتقول إن ده "أقسى عن قصد" من البروتوكولات المتساهلة في الأبحاث السابقة، وإنه بيطابق الواقع: في نظام حقيقي، رد مش قابل للاستخدام مش "إعادة محاولة ببلاش" — هو فشل في التصنيف. وفيه سبب تاني من الكود (R3 = النقطة رقم 3 في قايمة الإصلاحات المكتوبة في أول نوتبوك stage3b-repair): النسخة القديمة كانت بتحسب macro-F1 على العناصر اللي اتحلّلت بس، فالموديل اللي بيرد بكلام ملهوش معنى كان بيتحذف له العنصر من المقام — يعني الفشل كان بيتكافأ. دلوقتي الرقم الرسمي هو **strict** (unparseable = غلط).

**الـ idempotent repair pass**: نفس دالة الـ parser بتتطبّق مرتين — مرة وقت التوليد، ومرة تانية في مرحلة "إصلاح" بتعدّي على كل الردود الخام المؤرشفة وتعيد تحليلها. **Idempotent** يعني لو طبّقتيها مرة أو عشر مرات النتيجة واحدة. الهدف: "التنبؤ المخزّن والتنبؤ المتقيّم ما يقدروش يختلفوا في صمت" — الورقة بتقول إن نفس القاعدة بتتطبق وقت التوليد وفي مرحلة الإصلاح، ودالتا الـ parsing في المرحلتين 3 و3b بتنفّذوا نفس القاعدتين. وفي النتائج، الـ **11,518** دي كل الردود الخام اللي اتولّدت في دراسة الصياغة (الرقم من قسم الـ Discussion في الورقة)، والـ parser قبل 99.98% منها — يعني حوالي **2 رد بس** اترفضوا. ودي النقطة المهمة: لما Gemma قالت "security" على 96.7% من SecReq، الردود دي كانت **مظبوطة الشكل** وغلط في المعنى، مش ردود مبعثرة. الـ parser قبل 99.98% من الـ 11,518 رد بتوع دراسة الصياغة، فمعظم الأخطاء اللي هنشوفها هي ردود "صحيحة الشكل بس غلط" مش مشكلة تنسيق.
...
- **F1** = 2 × P × R ÷ (P + R): المتوسط التوافقي (harmonic mean) للاتنين؛ بيعاقب لو واحد منهم واطي. مثال يوضّح العقوبة: لو P = 1.0 وR = 0.10، المتوسط العادي كان هيبقى 0.55، لكن F1 = 2 × 1 × 0.1 ÷ 1.1 = **0.18**. عشان كده موديل بيقول "security" على كل حاجة (recall = 1 وprecision واطية) ما بياخدش مكافأة.
- **Accuracy** (نسبة الصح الكلية) = عدد الصح ÷ عدد الكل، بغض النظر عن الفئة. (وخلّي بالك: أنا بتجنّب كلمة "دقة" لوحدها لأنها بتترجم precision وaccuracy الاتنين، وهما حاجتين مختلفتين.)
...
خليني أحوّلهم لأعداد عشان تحسّيهم: فيه **125** عنصر security حقيقي في PROMISE. الـ recall = 0.888 معناه الموديل لقى منهم 0.888 × 125 ≈ **111** وفاته **14** (دول الـ FN). والـ precision = 0.8473 معناه إن اللي قال عليهم security كانوا 111 ÷ 0.8473 ≈ **131**، منهم 111 صح و**20** غلط (دول الـ FP). تأكيد: F1 = 2×111 ÷ (2×111 + 20 + 14) = 222 ÷ 256 = **0.867** — نفس الرقم بالظبط.
F1(security) = 2 × 0.8473 × 0.888 ÷ (0.8473 + 0.888) = 1.5048 ÷ 1.7353 ≈ 0.867.
وفئة non-security موجودة في نفس الملف (tab6) مش محتاجة استنتاج: precision = 0.9833، recall = 0.9763، **F1 = 0.9798** على 843 عنصر. يبقى macro-F1 = (0.8672 + 0.9798) ÷ 2 = **0.9235** — نفس الرقم اللي في tab9 بالظبط. (لاحظي إزاي الفئة الكبيرة سهلة والصغيرة هي اللي بتحدد المستوى.)
...
**قاعدة عامة تختصر كل الأمثلة دي:** لو الموديل بيقول فئة واحدة دايمًا ونسبتها في الداتا = p، يبقى precision = p، recall = 1، F1 = 2p ÷ (1 + p)، وmacro = F1 ÷ K (K = عدد الفئات المعلنة). جرّبيها بنفسك: 0.5413 → 0.702؛ 0.6014 → 0.751؛ 0.3541 → 0.523؛ 0.2887 → 0.448؛ 0.2385 → 0.385.
...
مثال صغير عشان تتخيّليها: لو عندك 5 عناصر أمنية [A, B, C, D, E]، عينة bootstrap ممكن تطلع [A, A, C, E, E] — يعني B وD اختفوا وA وE اتكرروا. بنحسب macro-F1 على العينة دي، ونكرر 2,000 مرة، وبعدين نرتّب الـ 2,000 رقم من الأصغر للأكبر: الحد الأدنى للفترة = الرقم رقم 50 (لأن 2.5% × 2000 = 50) والحد الأعلى = الرقم رقم 1,950. فلما نقول فترة BERT(w) = [0.8977, 0.9463]، ده معناه إن 1,900 عينة من الـ 2,000 وقعت ما بين الرقمين دول.

- **Stratified** (طبقي): السحب بيتعمل **جوه كل فئة حقيقية على حدة** — لو الفئة فيها 7 عناصر، بنسحب 7 مع الإرجاع من الـ 7 دول. ليه؟ تعليق الكود: الـ bootstrap العادي (i.i.d. = بنسحب كل عنصر من الكوربس كله من غير ما نبص لفئته) ممكن "يفقد فئة نادرة بالكامل، يدّيها 0، ولسه يقسم على K" — وده كان بيحصل فعلًا: على خلية Gemini all-11 (60 عنصر) كان بيختفي في المتوسط 1.37 فئة في كل إعادة عينة (لحد 5)، فالحد الأدنى للفترة كان بينزل لرقم مصطنع. الطبقي بيثبّت توزيع الفئات، وهو "الكمية اللي macro-F1 معرّف عليها".
- **seed 42**: الـ seed (الرقم اللي بيثبّت العشوائية)، عشان نفس الفترات تطلع لو حد أعاد التشغيل. (42 اختيار تقليدي في الكود العلمي، مفيش سبب إحصائي وراه.)
...
- Gemini، security، PROMISE: macro-F1 = 0.8287، CI = [0.6981, 0.9619]، عرض = 0.2638 — أكتر من 5 أضعاف عرض BERT (0.2638 ÷ 0.0486 = 5.4 ضعف).

**ليه فترة Gemini واسعة كده؟** لأن n = 60 بس (Gemini اتقيّمت على عينة صغيرة بسبب حصة الـ API). للمقارنة، دي أحجام الخلايا من عمود n في tab9: الإنكودرز 968 / 444 / 524 / 433 / 353 (الكوربس كله)؛ الموديلات المحلية 960 / 960 / 436 و491 / 415 / 341؛ Llama-3.3-70B (Groq) 290 / 294 / 286 و163 / 169 / 141؛ Gemini 60 في كل خلية (59 على top-4)؛ Nemotron 21. القاعدة: كل ما n يصغر، الفترة توسع، والأهم إن فيها **7 عناصر security بس** (عمود `min_class_support` في tab9). لما بنعمل bootstrap جوه فئة من 7 عناصر، تغيير عنصر واحد بيقلب F1 بتاع الفئة دي بشكل كبير، والـ macro بياخد نص وزنه من الفئة دي. مع 125 عنصر، عنصر واحد بيفرق أقل بكتير. الرسالة: رقم 0.8287 مش "غلط" بس **مش دقيق** — الحقيقة ممكن تكون في أي حتة من 0.70 لـ 0.96.

---

وآخر حاجة: الفترة دي مش مجرد رقم في ملف — هي **الخطوط السودا الصغيرة (error bars)** اللي بتشوفيها عند طرف كل عمود في أشكال RQ1 في عرض الرسالة، وهي عمودي `ci_lo` و`ci_hi` في tab9.

---

### 5) بوابة الـ reportability: 87 خلية، 6 مستبعدة، 81 مقبولة

**الخلية (cell)** = موديل × تاسك × كوربس × regime. مثلًا (BERT(w)، security، SecReq، cross_dataset) خلية؛ (Gemini، subtype_top6، PROMISE، prompted) خلية تانية. tab9 فيه 87 صف = 87 خلية، وتعالي نعدّهم مع بعض: BERT(w) وRoBERTa(w) عندهم **15 خلية لكل واحد** (FR/NFR في 3 أنظمة = 3، أمان PROMISE 3، أمان SecReq 3، و3 أنواع فرعية × نظامين = 6) → 30؛ BERT(u) وRoBERTa(u) **3 لكل واحد** (أمان PROMISE in-domain + cross-dataset في الاتجاهين) → 6؛ الـ 8 LLMs **6 خلايا لكل واحد** (كلها بنظام اسمه `prompted`) → 48؛ وNemotron 3. المجموع 30 + 6 + 48 + 3 = **87** ✓. خدي بالك إن خلايا الـ LOPO الاتنين موجودة في tab9 بس مش داخلة في عيلة الـ 260 اختبار اللي اتجرّب.
...
| 4 | Nemotron، fr_nfr | 21 | n<40; parse<0.5 | — (نسبة التحليل 0.0: ولا رد واحد اتفهم؛ والـ 17 رد اللي اتكلمنا عنهم فوق في حكاية الـ 18 صف هما من هنا بالظبط — الموديل بيبدأ يفكر "We need to decide…" ويتقطع عند 12 توكن) |
| 5 | Nemotron، security/PROMISE | 21 | class support<5; n<40 | security=2 |
| 6 | Nemotron، security/SecReq | 21 | n<40 | — |

لاحظي إن خلية Gemini all-11 برقم macro-F1 = 0.7889 وهو رقم "حلو" — بس 7 من الـ 11 فئة فيها أقل من 5 عناصر، فسبع أعشار الرقم ده مبني على عنصر أو اتنين. وخلية Llama-70B all-11 بـ 0.7524 على 163 عنصر سقطت بسبب فئة واحدة (maintainability = 4 — واحد أقل من الحد). Nemotron جاب 21 عنصر بس في كل خلية (3 × 21 = 63 تنبؤ، وده رقم الـ deck) بسبب حصة المزوّد، وفي FR/NFR ما اتفهمش منه ولا رد.

**ليه استبعاد مش تعويض (impute)؟** الورقة بتقول "excluded rather than imputed". التعويض يعني نخمّن رقم للخلية الناقصة أو نحسبها من العناصر اللي وصلت؛ لكن الفقد هنا مش عشوائي (الـ deck بيسميه "missing-not-at-random": حصص المزوّد هي اللي قطعت) فأي تخمين هيبقى منحاز. الاستبعاد بيخلّي الجداول أصغر بس صادقة، والخلية المستبعدة بتتلوّن رمادي في أشكال الرسالة اللي الكود بيرسمها؛ أما في الورقة نفسها فبتظهر كشرطة "—" في الجدول IV. وده ليه فيه "—" لخلايا Gemini top-6/all-11 وLlama-70B all-11 في الجدول IV (أما الشرطات في صفوف BERT_u/RoBERTa_u فسببها إن الكنترول ده ما اتدرّبش هناك أصلًا: اتعمل بس على أمان PROMISE in-domain وعلى cross-dataset).
...
**Paired** (مزدوج): الموديلين اتقيّموا على **نفس العناصر بالظبط**، متطابقين بمعرّف المتطلّب (id) مش بالترتيب. الكود بياخد تقاطع الـ ids بين الإنكودر والـ LLM، ولو أقل من 40 ما بيعملش اختبار (نفس ثابت `MIN_N = 40` بتاع البوابة). ولو b + c = 0 — يعني الموديلين ما اختلفوش في ولا عنصر — بيرجّع p = 1.0.
...
قبل الأمثلة الكبيرة، خدي أصغر حالة حقيقية وهنحسبها بإيدينا: BERT(w) in-domain مقابل Gemini على أمان PROMISE، n = 60، b = 5، c = 0 — يعني 5 اختلافات كلها لصالح الإنكودر. لو دي عملة عادلة اترمت 5 مرات، احتمال تطلع 5 صورة = (½)⁵ = 1/32، والاختبار ثنائي الاتجاه بيحسب كمان احتمال 5 كتابة = 1/32، فـ p = 2/32 = **0.0625** > 0.05. يعني بـ 5 اختلافات بس **مستحيل** توصلي لدلالة حتى لو الإنكودر كسبهم كلهم — وده بالظبط سبب إن 23 من 26 مقارنة مع Gemini طلعت غير حاسمة (وده مذكور في قسم Threats). حالة الـ 50/55 نفس الفكرة بالظبط بس بـ 105 رمية، والكود بيحسبها بـ scipy فتطلع p = 0.696.

McNemar بيبص **بس** على الخليتين المتعارضتين b وc (discordant). ليه؟ لأن a وd مش بتفرّق بين الموديلين — لو الاتنين صح أو الاتنين غلط، مفيش دليل إن حد أحسن. لو الموديلين متساويين فعلًا، فكل عنصر اختلفوا فيه ليه فرصة 50% يروح لـ b أو لـ c، زي عملة معدنية. فالاختبار الدقيق (**exact**) هو: احسب احتمال إن عملة عادلة مرمية (b + c) مرة تدّي نتيجة متطرفة زي اللي شفناها أو أكتر. في الكود: `binomtest(min(b, c), b + c, 0.5)` — اختبار binomial (ذات الحدين) ثنائي الاتجاه. "Exact" يعني الاحتمال محسوب بالظبط من توزيع ذي الحدين مش بتقريب كاي-تربيع، وده أنسب لأعداد صغيرة.
...
**مثال 2 (tab4) — حالة حاسمة:** BERT(w) in-domain مقابل Qwen2.5-3B على FR/NFR/PROMISE. n = 960، accuracy 0.9083 مقابل 0.5969. b = 344، c = 45. من 389 اختلاف، 344 راحت للإنكودر. احتمال عملة عادلة تعمل كده ≈ 4 × 10⁻⁵⁸ — الكود بيسجّلها **p = 0.0**. ودي محتاجة توضيح: ملف الـ CSV بيكتب p لحد 6 خانات عشرية بس، فـ "0.0" معناها p أصغر من 0.0000005 مش صفر بالظبط. فيه **98 اختبار من الـ 260** مكتوب p بتاعهم بالطريقة دي، وأصغر p غير صفري في الملف = 0.000001. الحكم: **Encoder wins**، وبيفضل كده بعد Holm وBH. (للاكتمال: a = 872 − 344 = 528، d = 43.)
...
خدي بالك من حاجة بتلخبط: **الطرف الـ LLM ثابت مش بيتحرك**. نفس تنبؤات Qwen2.5-7B على أمان PROMISE (خلية `prompted` واحدة) بتتقارن **تلات مرات**: مرة مع BERT(w) in-domain، ومرة مع BERT(w) cross-project، ومرة مع BERT(w) cross-dataset. اللي بيتغيّر هو الإنكودر بس — وده بالظبط معنى "الحكم بينقلب تحت الانزياح".
...
**ليه LOPO مش في الأسرة؟** LOPO (leave-one-project-out) هو تقسيم تاني لنفس تجربة cross-project على نفس الـ 968 متطلّب (موجود عشان جدول per-project). تعليق الكود: تنبؤاته بتتفق مع تقسيم grouped-CV على **~94%** من العناصر (يعني لو أخدنا نفس المتطلّب من الـ 968، الإنكودر المدرّب بطريقة LOPO والإنكودر المدرّب بطريقة grouped-CV بيدّوا نفس الـ label في 94% من الحالات — فالاختبارين شبه بعض)، فلو دخل الاتنين كانت الأسرة هتبقى 276 اختبار وفيها 16 اختبار "شبه مكرر" (2 إنكودر موزون × 8 LLMs على FR/NFR): 276 − 16 = 260. المشكلة مش بس التكرار: حجم الأسرة هو اللي BH وHolm بيقسموا عليه، فتضخيمه بنسخ مكررة بيغيّر التصحيح ويختبر نفس الفرضية مرتين. القاعدة: "تقسيم واحد لكل إنكودر".
...
- **Benjamini–Hochberg (BH)** (false discovery rate): بيضمن إن **نسبة** الإيجابيات الكاذبة وسط اللي قلنا عليهم دالّين ≤ 5%. أقل صرامة. الطريقة: رتّبي، اضربي الـ i-th في n/i، ثم خدي الحد الأدنى التراكمي من آخر الترتيب لأوله (من أكبر p لأصغر p).
...
مثال حقيقي على تأثير التصحيح من tab4: BERT(w) in-domain مقابل Gemini على security/PROMISE: n = 60، b = 5، c = 0، p خام = 0.0625 (مش دالّ أصلًا)، بعد BH = 0.0903، بعد Holm = 1.0. شوفي إزاي Holm رفع الرقم لـ 1.0: هو بيضرب في عدد الاختبارات اللي لسه ما اتفحصتش في الترتيب (عشرات)، وأي p معدّلة بتطلع أكبر من 1 بتتقص لـ 1.0.

ومثال حقيقي BH وHolm اختلفوا فيه (من tab4): RoBERTa(w) in-domain مقابل Llama-3.3-70B على FR/NFR في PROMISE: n = 290، b = 30، c = 11، p خام = 0.0043 → بعد BH = **0.0077** (دالّ، والحكم "Encoder wins") → بعد Holm = **0.493** (مش دالّ). ده واحد من الـ **39** اختبار اللي BH سابهم وHolm شالهم.

---

### 9) الحرّاس (sentinels) وفحص "زي بزي" (like-for-like)

**الحارسين (dummy sentinels):** الورقة بتشغّل "موديلين غبيين" على نفس السكورر بالظبط:
...
الغرض (من الـ deck): "أي موديل تحت الحارس بيتعلّم عليه، مش بيتعذر له". يعني لو موديل بـ 3 مليار باراميتر جاب أقل من "قول دايمًا NFR"، ده لازم يظهر مش يتخبّى. والورقة بتقول إن كل موديل مقبول عدّى الـ majority-class baseline. الأرقام الفعلية للـ stratified-random مش موجودة في tab0 (اللي فيه الأغلبية بس)، وما لقيتش كود بيطلّعها. تقدير مني (مش رقم منشور): لو رمينا "security" عشوائيًا بنسبة 12.9% على PROMISE، هتطلع precision ≈ recall ≈ 0.129 → F1(security) ≈ 0.13، وF1(non-security) ≈ 0.87 → macro ≈ **0.50**؛ ونفس الحاجة على FR/NFR. يعني الحارس العشوائي بيقعد حوالي 0.5 على أي تاسك ثنائي. **لو حد سألك في المناقشة:** قولي إنه معرّف في الورقة كحارس، والرقم المنشور في الملفات هو حارس الأغلبية بس.
...
من tab10 على top-4: 59 عنصر مشترك (min support في التقاطع = 10 لكل فئة، فوق حد البوابة): RoBERTa(w) = 0.966، Llama-3.3-70B = 0.9298 → الفرق +0.036 لصالح الإنكودر، وهو الرقم اللي في الورقة ("0.966 against 0.930"). الخلاصة: الترتيب مش بسبب اختلاف التغطية. الجدول بيعمل نفس الحاجة على باقي التاسكات (60 عنصر مشترك للثنائيات، 218 لـ all-11، 141 لـ top-6). وليه 60 بالذات في الثنائيات؟ لأن Gemini جاوبت على 60 بس، فالتقاطع بيبقى هو عناصر Gemini. وعلى الـ 60 دول في أمان PROMISE فيه **7 عناصر security بس** (عمود `intersection_min_support`)، وBERT(w) جابت 1.0 وBERT(u) وRoBERTa(w) 0.9619 — يعني الترتيب ثابت بس محسوب على 7 عناصر أمنية، وده حد لازم تعرفيه وبيسمّي الموديلات اللي اتشالت من التقاطع: Nemotron في التلات كتل الثنائية (ومعاه BERT/RoBERTa unweighted على SecReq لأنهم ما اتدرّبوش in-domain هناك)؛ Gemini من top-6؛ وGemini وLlama-70B من all-11 لأن خلاياهم سقطت من البوابة (كتلة top-4 ما اتشال منها حد).

الكود بيحكي كمان إن نسخة قديمة من الجدول ده كانت شالت الإنكودرز من التقاطع بالغلط، فطلع الـ LLM "فايز" في التلات كتل بتاعة الأنواع الفرعية (top-4 وtop-6 وall-11): علامة `in_core` كانت NaN عند صفوف الإنكودرز، فاتحوّلت للنص "nan" واتحسبت False، فالإنكودرز اتشالوا من التقاطع بالغلط — وإن RoBERTa بـ 0.9660 مقابل 0.9298 هو اللي قلب الوضع لما اتصلّح. ده مثال حي على ليه الفحص ده لازم يتعمل بشكل صريح مش بالافتراض.

---

### خلاصة تاخديها في إيدك

- البرومبت 3 أسطر + مكان أمثلة + السؤال؛ التلات صياغات بتغيّر أول كتلة بس؛ terse في تاسك الأمن سؤال نعم/لا وده سبب الـ 0.485.
...
### اقري ده الأول: الفكرة كلها بمثالين من الحياة

الجزء ده هو أصعب جزء في الورقة، فخلّينا نفهمه بالبلدي الأول وبعدين ندخل على الأرقام.

#### أولًا: الـ prior — كل الحكاية في سؤال واحد

> **من كل 100 متطلّب، كام واحد أمني؟**

الإجابة على السؤال ده اسمها الـ **prior**. وليها تلات أسامي (prior / base rate / prevalence) — نفس الحاجة بالظبط، بس كل مجال بيسميها باسم.

- في **PROMISE**: من كل 100 متطلّب، حوالي **13** أمني. يعني الأمن **نادر**.
- في **SecReq**: من كل 100، حوالي **40** أمني. يعني الأمن **شايع**.

الفرق تلات أضعاف، ودي مش تفصيلة جانبية — دي محرّك النتيجة الرئيسية في الورقة كلها.

**مثال من الحياة يخلّيها تلزق:** تخيّلي دكتور اشتغل 10 سنين في عيادة في منطقة موبوءة، **40%** من اللي بيجوا عنده عندهم المرض فعلًا. اتعوّد إن أي عرض بسيط معناه غالبًا المرض موجود — وده كان **صح** في المكان ده.

بعدين اتنقل لعيادة تانية، فيها **13%** بس عندهم المرض. لو فضل بنفس الحساسية، هيقول "عندك المرض" لناس كتير أوي **غلط**.

هو مش بقى غبي، ومنسيش الطب. هو بس **جايب معاه توقّع من المكان القديم** مش مناسب للمكان الجديد.

ده حرفيًا اللي حصل لـ BERT: اتدرّب على SecReq (40% أمن)، اتنقل على PROMISE (13% أمن)، فقال "أمان" على **44.2%** من المتطلبات — رقم قريب من الـ 40% بتاع مكانه القديم، مش من الـ 13% الحقيقي.

#### والأرقام التلاتة 12.9% و12.6% و12.7% دي إيه؟

**دول نفس الحاجة، مفيش تناقض.** الفرق إننا بنقسم على مقام مختلف كل مرة:

| الرقم | الكسر | ده محسوب على إيه؟ |
|---|---|---|
| **12.9%** | 125 ÷ 968 | الكوربس كله |
| **12.6%** | 121 ÷ 960 | بعد ما شيلنا 8 متطلبات استخدمناهم كأمثلة جوّه البرومبت (منهم 4 أمنية) |
| **12.7%** | 38 ÷ 300 | جوّه العينة الصغيرة بتاعة دراسة الصياغة |

زي لو سألتك "نسبة البنات في الفصل؟" — الرقم هيفرق شوية لو حسبتيه على الفصل كله ولا على اللي حضروا النهارده. نفس الحقيقة، عيّنة مختلفة. **ولو اللجنة سألت:** قولي رقم واحد بس ووضّحي على إيه ("12.9% على الكوربس الكامل") وكفاية.

#### ثانيًا: الـ distribution shift — الموديل اتدرّب في دنيا واتقاس في دنيا تانية

خلّينا نتخيّل مفتّش أمن اشتغل في **مطار** واتنقل لـ **محطة قطر**. تلات حاجات ممكن تكون اتغيّرت:

1. **نسبة الحالات اتغيرت** (*prior shift*): في المطار 40% من الشنط تستدعي الوقوف، في المحطة 13% بس. لو فضل بنفس الحساسية هيوقف ناس كتير على الفاضي.
2. **شكل الحاجات نفسها اتغير** (*vocabulary / content shift*): المطار شنط سفر ولابتوبات، المحطة شنط أكل وأكياس. الأشياء نفسها مختلفة، مش بس نسبتها.
3. **تعريف "مشبوه" نفسه اتغير** (*definitional shift*): القاعدة نفسها بقت أوسع أو أضيق. الحكم اتغير، مش الداتا.

التلاتة موجودين مع بعض في ذراع الـ cross-dataset، متراكبين فوق بعض — وعشان كده لازم نفرّقهم.

#### وإزاي عرفوا إن اللي حصل هو النوع الأول أساسًا؟

دي أحلى حتة في الورقة، والدليل بسيط جدًا:

**لو المشكلة كانت "مش فاهم الكلام الجديد" (النوع التاني):** الموديل كان هيبقى تايه و**هيفوّت** متطلبات أمنية كتير، يعني الـ **recall** (كام لقى من الحقيقيين) هو اللي كان هينهار.

**اللي حصل العكس بالظبط.** بالأرقام:

- فيه **125** متطلّب أمني حقيقي في PROMISE.
- الموديل قال "أمان" على **428** متطلّب (44.2% من 968).
- منهم **90 صح**: يعني لقى 90 من الـ 125 → **recall = 0.72**، وده كويس ومقبول.
- بس التمن: **338 غلط**. من كل 100 قال عليهم أمان، **21 بس** صح → **precision = 0.21**.

> **الجملة اللي تحفظيها:** هو مش عاجز يلاقيهم — هو عاجز **يبطّل** يلاقيهم. ودي بصمة الـ prior shift، مش بصمة سوء الفهم.

#### وأمانة الورقة: ما قالتش إن ده السبب الوحيد

ودي نقطة قوة لازم تعرفيها لو اتسألتي. في **الاتجاه العكسي** (تدريب على PROMISE بـ 12.9% ← اختبار على SecReq بـ 39.9%): لو الـ prior لوحده هو اللي شغال، الموديلات كانت هتتوقّع حوالي 12.9%. اللي حصل إنهم توقّعوا **33.6%** و**29.1%** — يعني اتسحبوا لفوق ناحية الحقيقة. معنى كده إن فيه حاجة في **النص نفسه** بتشتغل وبتساعدهم.

عشان كده الورقة بتقول بالحرف إن الـ prior هو *"الحد المسيطر في أكبر انهيار شفناه"* — **مش** "السبب الوحيد"، وعلى **زوج كوربسات واحد** بس.

وفيه كمان الـ **definitional shift**: كلمة security نفسها معرّفة مختلف في الكوربسين. في PROMISE اللابل مشتق من فئة SE اللي هي غير وظيفية بالتعريف؛ في SecReq بيعلّموا أي جملة ليها علاقة بالأمن حتى لو وظيفية. (مثال يوضّح الفكرة، من عندي مش من الورقة: جملة زي "النظام لازم يطلب باسورد عند الدخول" — في SecReq دي أمنية، وفي PROMISE دي وظيفية فمش هتدخل فئة SE أصلًا.)

#### لو اتسألتي في المناقشة

| السؤال | الإجابة في سطرين |
|---|---|
| إيه الـ prior؟ | نسبة الفئة في الداتا. الأمن 12.9% في PROMISE و39.9% في SecReq — تلات أضعاف. |
| منين عرفتوا إنه prior مش vocabulary؟ | لأن الـ precision انهارت (0.847 → 0.21) والـ recall صمد (0.888 → 0.72). لو كانت مفردات، الـ recall هو اللي كان هيقع. |
| يعني الـ prior هو السبب الوحيد؟ | لأ. في الاتجاه العكسي الموديلات توقّعت 33.6% مش 12.9%، يعني المحتوى بيساهم. الورقة بتقول "الحد المسيطر في أكبر انهيار" بس. |
| والـ definitional shift؟ | تعريف الأمن مختلف بين الكوربسين، ومذكور في Threats كحد من حدود الدراسة. |

خلاص، دلوقتي بعد ما الفكرة وصلت، نمشي على الشكل 4 لوحة لوحة بالتفصيل.
