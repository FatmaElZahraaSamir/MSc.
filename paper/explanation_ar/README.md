...
- أحجام عينات الـ prompted: 300 لكل خلية ثنائية و200 لكل خلية sub-type. ليه 300؟ الكود بيقول: أكبر n بحيث حصة يوم واحد من API مجاني (900 طلب) تغطي التلات مجموعات: 3 × 300 = 900؛ وعند n = 300 فرق ~0.08 في macro-F1 يبقى قابل للكشف عند p < 0.05. وليه all-11 طلع 218 مش 200؟ عشان الحد الأدنى 10 لكل كلاس اتطبّق بعد التوزيع النسبي فزوّد 18. الموديلات المحلية الستة جاوبت كمان على الـ frames الكاملة (960 / 960 / 436).
...
**F1 per class**
- المتوسط التوافقي لـ precision وrecall لكلاس واحد: F1 = 2PR/(P+R). لكلاس security cross-dataset: 2×0.2103×0.72/(0.2103+0.72) = 0.3255 (tab6).
...
- اختبار بيبص بس على الـ items اللي الموديلين اختلفوا فيها: b = عدد اللي الـ encoder جابها صح والـ LLM غلط، c = العكس. اللي الاتنين صح فيها أو غلط فيها ما بتدخلش. "exact" يعني بنحسب الاحتمال بالظبط بتوزيع binomial، مش تقريب، وده أنسب للأعداد الصغيرة: p = binomtest(min(b,c), b+c, 0.5).
...
**Holm correction**
- الأشد: بيرتّب الـ p-values وبيقارن أصغرها بـ 0.05/260، والتالية بـ 0.05/259، وهكذا. بيتحكم في احتمال حصول ولو خطأ واحد (FWER). النتيجة: 131 significant من 260.
...
| 12.6% / 39.7% / 54.2% | نفس النسب على frames الـ prompted | بعد شيل 8 أمثلة: 121/960، 173/436، 520/960 |
| 107 | عدد الـ folds | 30 + 28 + 47 + 2 |
| 87 / 81 | خلايا قبل وبعد الـ gate | 6 ساقطة |
| 76,115 | كل الـ predictions | 24,280 encoder + 35,049 prompted binary + 16,786 prompted sub-type |
...
- **8 صناديق** ماشية من الشمال لليمين — 6 منهم بخلفية رمادية فاتحة (المصدرين، Unified، Frozen، Strict scoring، Analysis) وصندوقين الذراعين بخلفية بيضا وإطار ملوّن (أزرق متصل للإنكودر، ذهبي مقطّع للـ prompted): صندوقين فوق بعض على أقصى الشمال (المصدرين)، بعدين "Unified corpus"، بعدين "Frozen splits"، بعدين — في النص — صندوقين فوق بعض (الذراعين)، بعدين "Strict scoring"، وآخر حاجة "Analysis".
...
- **47 projects**: PROMISE_exp مجمّع من 47 مشروع مختلف، وكل متطلب عليه رقم المشروع بتاعه (عمود `project` في `unified.csv`؛ الأرقام من 1 لـ 49 مع فجوات، المهم إن عددهم 47). الرقم ده مهم جداً بعدين لأنه هو اللي هيتعمل عليه الـ grouping في الـ cross-project والـ LOPO (هنشرحهم تحت). المشاريع دي أحجامها متفاوتة جداً: أكبر مشروع فيه 92 متطلب، وفيه مشروعين كل واحد فيه متطلب **واحد بس** (مشروع 22 ومشروع 39) — خدي بالك من المعلومة دي لأنها هتفسّر حاجة في الـ LOPO.
...
- **510**: عدد الصفوف الخام في SecReq (Knauss et al. 2011). بس SecReq مش ملف واحد؛ هو **3 ملفات**، كل ملف "مواصفة" (specification) صناعية لنظام أمني:
  ...
  - **GPS**: 176 صف.
  - 124 + 210 + 176 = **510** ✓.
  - وفي ملف GPS كان فيه سطر واحد الـ label بتاعه مشوّه (مش "sec" ولا "nonsec" بشكل مفهوم) فاتشال: الـ manifest بيقول `malformed_labels_dropped: 1` و `GPS: skipped 1`. السطر ده مش داخل في الـ 510.
...
- **BERT, RoBERTa**: الاتنين هما `bert-base-uncased` (0.11 مليار parameter) و `roberta-base` (0.13 مليار) من جدول 3.
- **± class weighting**: كل واحد فيهم ليه نسختين — **(w)** weighted: في الـ loss بنكبّر وزن الـ class النادر بعكس تكراره (inverse-frequency / balanced) عشان الموديل ما يتجاهلوش، و **(u)** unweighted: من غير الوزن ده، كـ control. النسخة (u) ما اتعملتش في كل حاجة، اتعملت على تاسك الأمان بس (in-domain على PROMISE والـ cross-dataset في الاتجاهين) — وهنشوف ده بالأرقام في حساب الـ 24,280 تحت.
- **re-fitted per regime**: الـ encoder بيتدرّب **من جديد** في كل محطة على training folds المحطة دي. يعني BERT بتاع in-domain غير BERT بتاع cross-project غير BERT بتاع cross-dataset — نفس الـ checkpoint الأصلي، بس اتعمله fine-tuning على داتا مختلفة. وده بالظبط اللي بيخلّي الذراع ده **يتحرّك** على محور الـ regime.
...
- **8 LLMs, 3 tiers**: 8 موديلات في 3 "طبقات" (tiers) حسب طريقة الوصول (جدول 3):
  - **local open** — 6 موديلات مفتوحة شغّالة محلياً على كارت T4 مجاني بضغط 4-bit: Gemma-2-2B، SmolLM3-3B، Qwen2.5-3B، Phi-4-mini، Qwen2.5-7B، Llama-3.1-8B (أحجامهم من 2.61 لـ 8.03 مليار parameter).
  - **hosted** — موديل واحد: Llama-3.3-70B عن طريق API مجاني (Groq).
...
- **Sec. prev.** (security prevalence) = نسبة العناصر اللي في الـ class الإيجابي (security) لتاسك الأمان. الحساب:
  ...
  - Unified: (125 + 177) / 1,412 = 302 / 1,412 = 0.2139 → **21.4%** ✓.
...
**ليه SecReq = 3 folds؟** لأن الـ "project" في SecReq هو مواصفة صناعية كاملة، وعندنا **3** بس: CEPS (ePurse)، CPN، GPS. الكود بيحسب `k = min(n_folds, n_groups)` = min(5, 3) = **3**، وبيطلع warning "only 3 project(s); reducing to 3 folds". يعني كل fold بيحجز مواصفة كاملة كـ test ويدرّب على الاتنين التانيين؛ متوسط الـ train = 444 × 2/3 = **296** item (وده الرقم اللي في `summary_finetuned.csv`). قبل الـ dedup الأحجام كانت CEPS 124، CPN 210، GPS 176 (من الـ manifest).
...
**ليه موجودة أصلاً** وإحنا عندنا cross-project بـ 5 folds؟ تعليق الكود: 47 نقطة لكل مشروع هي اللي بتخلّي "variance statement" ممكن: "الـ encoder بيخسر كذا على المشروع الوسيط وكذا على الأسوأ". رقم pooled واحد بيخبي بالظبط ده (وده اللي طلع في tab11b: كل موديل عنده مشروع بـ 0.0 ومشروع بـ 1.0).

**ليه pooled مش per-fold؟** لأن الـ folds دي مش قابلة للتقييم فرادى: من الـ 47 test set، **24** فيهم class واحد بس (كله FR أو كله NFR)، و**2** فيهم requirement واحد بس. لو حسبتي macro-F1 لكل fold: الـ class الغايب هياخد F1 = 0 وبرضه هنقسم على 2 → تحيّز منهجي لتحت سببه حجم الـ fold مش الموديل. فالحل: نجمّع الـ 968 prediction (كل requirement اتختبرت مرة واحدة بموديل ما شافش مشروعها) ونحسب رقم واحد. الأرقام الـ pooled دي — 0.8549 (BERT) و0.8592 (RoBERTa) — موجودة في tab9_evaluability.csv وsummary_finetuned.csv؛ وخدي بالك إن Table V نفسها ما فيهاش صف LOPO: عمود cross-project فيها هو الـ grouped arm (0.843 / 0.853)، والورقة بتستخدم LOPO بس في فقرة "aggregate scores hide per-project failure".
...
**ليه security بس؟** الـ FR/NFR labels موجودة في PROMISE بس، والـ sub-type labels في PROMISE بس؛ SecReq عندها security بس (الـ manifest بيقول `label_fr_nfr: {}` لـ SecReq). فـ security هي الـ task الوحيدة اللي الكوربسين بيحملوها بـ classين. الكود بيسجّل كل task اتخطّت بالاسم، عشان الغياب يبقى نتيجة مُبلَّغة مش فجوة صامتة. وتعليق الكود بيحذّر من حاجة الورقة بتقولها برضه: PROMISE بيعرّف security كـ sub-type من NFR (فبالبناء non-functional)، بينما SecReq بيعلّم أي جملة ليها علاقة بالأمان حتى لو functional، فجزء من اللي بنقيسه definitional shift مش بس distribution shift.

### 7) التلات "محطات" و"encoders re-fitted per station" يعني إيه عملياً؟

**regime** = نظام تقييم = مستوى الـ distribution shift (اختلاف الداتا اللي بنختبر عليها عن اللي دربنا عليها). التلات محطات بالترتيب: in-domain (نفس الكوربس، خلط عشوائي) → cross-project (مشاريع غير مشوفة) → cross-dataset (كوربس تاني بالكامل).

**"re-fitted per station"** عملياً معناه: عند كل عيلة، الـ encoder بيتدرّب **من الصفر** (من الـ checkpoint الأساسي bert-base أو roberta-base) على الـ train بتاع كل fold، ويتنبأ على الـ test بتاعه، ويتشال. يعني:
...
الوقت: BERT على in-domain FR/NFR أخد 273.5 ثانية للـ 5 folds ≈ 55 ثانية للموديل الواحد على T4؛ LOPO أخد 3,149 ثانية للـ 47.
...
**الـ frame أولاً (960 / 960 / 436).** قبل أي عينة، الكود بيشيل من كل خلية الـ **few-shot exemplars** (الأمثلة اللي بتتحط في الـ prompt). الـ pool = **4 أمثلة لكل class** (config: `fewshot_pool_per_class: 4`؛ ليه 4؟ لأن أكبر k في السلم هو 8 على task ثنائية = 4 لكل class، فلازم الـ pool يكفي). فلكل task ثنائية: 2 classes × 4 = **8** items بتتشال، عشان مثال في الـ prompt عمره ما يبقى test item. وده مصدر الـ 960 = 968 − 8 على PROMISE، و436 = 444 − 8 على SecReq. الـ "frame" ده هو مجموعة الأسئلة الكاملة للذراع الـ prompted. (للـ sub-types الـ pool = 3 لكل class لأن k ≤ 2 لكل class: all-11: 524 − 33 = 491؛ top-6: 433 − 18 = 415؛ top-4: 353 − 12 = 341؛ وكلها موجودة في tab9 كـ n للموديلات المحلية.)

ملحوظة: الـ 8 items دي بتفضل بتتقيّم عند الـ encoders (968 مش 960)، عشان كده الـ n بتاع encoder 968 وبتاع Gemma 960 في نفس الخلية؛ الـ McNemar بيتعمل على التقاطع.

**الـ core sample (300 لكل خلية ثنائية).** الـ API models عندها quota يومية مجانية، فمش هينفع كل موديل يجاوب على 960. الكود بيعرّف subset ثابت اسمه CORE: `core_eval_n: 300`. **ليه 300 بالذات؟** تعليق الكود حرفياً: 300 هو أكبر n بحيث الـ quota اليومية المجانية الكاملة لمزوّد API واحد (900 request) تغطي التلات خلايا الثنائية: 3 × 300 = **900**. وبيضيف: عند n = 300، فرق macro-F1 حوالي **0.08** بيبقى قابل للكشف عند p < 0.05، وده أقل بكتير من الفروق الملحوظة بين الموديلات. (الكود بيقول الجملة دي كتبرير power ولا بيوضح الحسبة؛ فخديها كما هي مصدرها.)
...
- Llama-3.3-70B (hosted على Groq): **290 / 294 / 286** على الثنائي، و**169 / 141 / 163** على top-4 / top-6 / all-11. قريبة من 300 و200 مش بالظبط؛ الكود بيرجع النقص لـ rate limiting في الطبقة المجانية بتاعة Groq (notebook الإصلاح بيقول إن 145 صف من الـ sub-types ضاعوا كده، وإن الفقد مش عشوائي: maintainability خسر 60% من عناصره)، وإن تغطية الـ API models "بتتبلّغ" مش بتتفرض.
- Gemini 3.1 Flash-Lite (commercial): **59–60** في كل خلية. السبب المذكور في الكود: quota Gemini المجانية = **180** request/يوم مقابل خطة 900، و180 ÷ 3 = 60.
...
**4-bit NF4 على T4:** الـ 6 موديلات المحلية بتشتغل مضغوطة (quantised) لـ 4 bits على كارت NVIDIA T4 مجاني (16 GB). تعليق الكود: Llama-3.1-8B في fp16 محتاج ~16 GB ومش هيدخل؛ وبيخلّي الدقة موحّدة لكل الموديلات عشان ما يبقاش فيه confound (الصغير fp16 والكبير 4-bit).

**الـ hyper-parameters بتاعة الـ encoders**، سطر لكل واحد:
- **AdamW**: الـ optimiser (خوارزمية تحديث الأوزان)؛ نسخة من Adam بـ weight decay منفصل، وهي الافتراضية لـ fine-tuning الـ BERT.
...
- **batch 16**: كام requirement بيتحسب عليهم gradient واحد. (`train_bs: 16`.)
...
ليه سقطوا؟ افتكري إن العيّنة بتتوزّع **بنسبة الـ class prior** (proportional): availability هي 31 من 433 = 7.2% من top-6، وعيّنة Gemini 60 عنصر بس، يبقى نصيبها الطبيعي حوالي 60 × 0.072 ≈ 4 عناصر — والـ artefact بيسجّل إن اللي وصل فعلاً 2. الورقة ما بتقولش ليه 2 بالذات (الـ Threats بتذكر بس إن حدود الـ free tier سبّبت dropout في خلايا مستضافة)، بس المهم: عنصرين ما ينفعش تحسبي عليهم F1 لـ class، فالخلية كلها ما تتكتبش. الورقة اختارت **الاستبعاد بدل التخمين** ("excluded rather than imputed"). خدي بالك: الأرقام اللي في العمود الأخير في الجدول اللي فوق (0.789، 0.752) موجودة في الـ artefact بس **مش في الورقة**، وأنا حاطّاها لك عشان تفهمي إن الـ gate ما بتشيلش الموديلات الوحشة، بتشيل الخلايا اللي رقمها مش موثوق.
...
**(3) ميزة الـ encoder بتتوسّع مع حجم الـ label set.** الحسبة من الـ artefact بـ 4 خانات عشرية (الورقة والـ deck بيقرّبوا لـ 3):
...
- *Security PROMISE*: الأربع encoders (0.923، 0.907، 0.903، 0.902) وGemma (0.900، CI [0.870, 0.929]) وLlama-70B (0.891، [0.843, 0.937]) وQwen2.5-7B (0.884، [0.855, 0.911]) كلهم متداخلين مع بعض — عشان كده 18 تعادل في العمود ده. اللي برّه الكتلة دي بوضوح: Phi-4-mini 0.798 [0.771, 0.827] وQwen2.5-3B 0.707 [0.654, 0.754].
...
**ليه المحور الأفقي بيقول "identical test items"؟** لأن الـ 3 نقط بتوع BERT (w) كلها محسوبة على **نفس الـ 968 عنصر** من PROMISE_exp. اللي بيتغيّر هو **إيه اللي اتدرّب عليه الموديل**:
...
- BERT (u): (0.5341 − 0.9069) / 0.9069 = −0.3728 / 0.9069 = **−41.1%** → −41. الشرطة في cross-project لأن النسخة الـ unweighted اتعملت كـ control على عيلتين بس: in-domain PROMISE وcross-dataset (الورقة في قسم Model configurations بتقول كده صراحة)، فمفيش لها cross-project أصلاً.
...
- وده بيفسّر الـ 11 فوز للـ LLMs في cross-project اللي شفناهم في لوحة (b): كلهم على security/SecReq، لأن الـ encoder هناك وصل لـ 0.588 بينما Qwen2.5-7B مثلاً على 0.846 من غير تدريب.
...
| Phi-4-mini | 0.7985 | 0.6353 | **+0.1632** |
  ...
  فالمدى "−0.034 لـ +0.163" هو من Qwen2.5-3B لـ Phi-4-mini. الورقة بتحط الرقمين دول جنب −0.3956 عشان تقول: "حتى لو حسبنا للـ LLMs أسوأ فرق ممكن بين الكوربسين، عمره ما يوصل لنص خسارة BERT". بس لازم تقوليها في المناقشة بوضوح: **الكميتين مش نفس الحاجة**، وعشان كده الـ deck بيكتب [cross-dataset] و[corpus delta] كعلامات مختلفة في الرسمة.
...
3. جدول 5: cross-project بيكلّف 3–12% على PROMISE_exp (−3.0 لـ −11.9) و30% على SecReq (−29.6، لأن 3 مواصفات بس)؛ cross-dataset بيكلّف BERT 42.8% وRoBERTa 22.7%، والاتجاه العكسي 17.4%.
...
الأرقام في القسم ده جاية من أربع مصادر: الشكل 4 في الورقة (`fig_prior_shift.png`)، الكود اللي رسمه (`make_figs.py`، دالة `fig_prior_shift`)، وثلاث جداول من Stage 4: `tab6_perclass.csv` (precision/recall لكل فئة)، `tab8_prompt_sensitivity.csv` (الصياغة)، و`tab7_fewshot.csv` (الـ few-shot).
...
الكود بيكتب الرقم فوق العمودين بس في المحطة الأخيرة ("label only the decisive pair"): بتشوفي **0.2** فوق الأزرق و**0.72** فوق المخطط. الرقم الدقيق في `tab6_perclass.csv` هو 0.2103، والرسم بيقرّبه لخانتين.
...
- **موديلين**: Qwen2.5-7B وGemma-2-2B. (الورقة ما بتقولش ليه الاتنين دول بالذات؛ من Table IV واضح إن Qwen2.5-7B هو الأقوى محليًا وGemma-2-2B من الأصغر، فالمنطق المعتاد إنهم بيمثّلوا طرفي الحجم — وده تفسيري.)
...
2. **Security**: اللوحة دي فيها **خليتين لكل موديل** (security على PROMISE وsecurity على SecReq، الاتنين بنفس اللون)، وده تفسيري للخطوط الرأسية الصغيرة اللي بتشوفيها عند k = 4 وk = 8: بتوصّل قيمتي الكوربسين لنفس الموديل عند نفس k. من الـ CSV: SmolLM3-3B (بني) بينهار على PROMISE من 0.8739 لـ 0.6088 عند k = 4، وعلى SecReq من 0.733 لـ 0.6004 عند k = 8. Gemma (أزرق) بتنزل في الكوربسين (PROMISE 0.8895 → 0.7844، SecReq 0.8271 → 0.7398 عند k = 8). أما Phi-4-mini (أخضر) وQwen2.5-3B (أحمر) بيطلعوا: Phi على SecReq 0.67 → 0.8488 عند k = 2، وQwen2.5-3B على PROMISE 0.7107 → 0.8883. وQwen2.5-7B (بنفسجي) وLlama (برتقالي) شبه مسطّحين فوق 0.85.
...
- **الـ encoder:** الوقت لكل item حوالي 0.016 ثانية (ده الـ latency اللي هنشوفها تحت). إذن:
  0.016 × 0.5260 ÷ 3600 = 0.016 × 0.000146 ≈ **$2.4e-6** لكل item (يعني حوالي 2.4 في المليون من الدولار؛ لو حسبتيها بالآلة بتطلع 2.34e-6، والملف بيكتب 2.4e-6 لأنه بيحسب من الـ latency الفعلي 0.0161 ثانية وبيقرّب لرقمين). اضربي في 1,000 → **$0.0024 لكل 1,000 تصنيف**. ده الرقم اللي هتشوفيه في كل مكان في الورقة (0.0024). في ملف cost1 بعض خلايا الـ encoder مكتوبة 0.0023 وبعضها 0.0024، لأن الـ latency الفعلي بيتراوح بين 0.0157 و0.0163 ثانية والتقريب لأربع خانات عشرية بيطلع 0.0023 أو 0.0024؛ الـ median على الـ 36 خلية encoder = 0.0024 (ده enc_med في cost_stats.json).
...
- **موديلات الـ API:** دي ما بتتسعّرش بالوقت، بتتسعّر بالـ tokens بسعر القائمة الرسمي (list price): Gemini 3.1 Flash-Lite **$0.25 لكل مليون token input و$1.50 لكل مليون token output**، وLlama-3.3-70B على Groq **$0.59 / $0.79**. كل خلية بتتحسب: (tokens_in × سعر الـ input + tokens_out × سعر الـ output) ÷ مليون ÷ عدد الـ items × 1,000. مثالين حقيقيين من cost1 على SecReq security:
  - Llama-3.3-70B: 35,161 token input و703 token output على 286 item (مش 436، لأن الـ free tier وقّع جزء من الطلبات؛ الخلية عدّت الـ gate رغم كده). الحسبة: (35,161 × 0.59 + 703 × 0.79) ÷ 1,000,000 = (20,745 + 555) ÷ 1,000,000 = $0.0213 للـ 286 item → ÷ 286 × 1,000 = **$0.0745 لكل ألف** ✓.
...
- فالخط بيبدأ عند 0.0080 (عند n = 1 التكلفة 0.0080 + 0.0000024 ≈ 0.0080) وبيفضل مسطّح تقريباً لحد كام مية item، وبعدين يبدأ يطلع ببطء. عند n = 10,000: 0.0080 + 0.024 = $0.032. في الرسم مكتوب جنبه "BERT (w), incl. $0.0080 training".
...
- خدي بالك إن جدول "Cost and latency by tier" (موجود في paper/tables/tab_cost.tex وفي slide 37 من دفاعك، مش داخل نص الورقة النهائية) بيجمّع **بالمستوى**: local 0.0312–0.0522، hosted 0.0704، commercial 0.0248. دي أرقام لكل موديل مجمّعة على خلاياه، عشان كده شكلها مختلف عن 0.0216–0.1132 اللي بيغطي كل خلية على حدة. الاتنين صح، بس من مستويين تجميع مختلفين (وسلايد 19 من دفاعك بتقول كده بالحرف).

**الـ Pareto frontier على مستوى الورقة كلها (cost2_pareto.csv): 10 خلايا، 9 encoders + 1 prompted.** هنا الـ frontier محسوب على كل الـ 81 خلية reportable مع بعض (كل task وكل regime)، مش جوّه خلية واحدة زي اللوحة (a):
...
- **الـ 70B المستضاف (Groq): 0.095 ثانية** = الـ median على خلاياه (على الـ binary tasks 0.094–0.095، وعلى الـ sub-types 0.214–0.216 لأن الـ prompt أطول بسبب عدد الـ labels). أسرع مستوى prompted، لأن Groq شغّال على hardware متخصص، رغم إنه أكبر موديل (70.6 مليار parameter حسب cost1).
...
BERT_w على SecReq: in-domain 0.8356، cross-project 0.5883 → (0.5883 − 0.8356) ÷ 0.8356 = −29.6% ≈ −30. السبب البنيوي: SecReq فيه 3 مجموعات مشاريع بس (المواصفات CEPS، CPN، GPS)، فالـ cross-project = 3 folds، وكل fold بيحجز مواصفة كاملة، يعني الموديل بيخسر تلت داتا التدريب وبيتقاس على وثيقة مختلفة تماماً في الأسلوب. في PROMISE (47 مشروع، 5 folds) الخسارة 3–12% بس.
...
لأن على الـ free tier الـ mean بيقيس طابور المزوّد مش الموديل. مثال من الكود: Llama-3.3-70B على Groq في FR/NFR: median 0.094 ثانية، لكن mean 2.070 ثانية، p95 = 0.719، وأقصى طلب 59.194 ثانية. الورقة بتقول إن الـ means المستضافة متضخمة "لحد مرتبتين عشريتين" (الـ CSV: 1.6–15.9 ثانية). فالتوقعات بتستخدم الـ median (encoders 0.016، hosted 0.095، local 0.19–0.36، commercial 0.61 ثانية) والـ mean بيتبلّغ جنبه مع flag لما mean > 3 × median.
